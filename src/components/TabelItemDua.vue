      ...
      <table v-if="filteredData.length > 0">
        ...
        <tbody>
          <tr v-for="(data, index) in paginatedData" :key="index">
            <td>{{ data.provinsi }}</td>
            <td>{{ data.kota }}</td>
            <td>{{ formatDate(data.parameter[1].date) }}</td>
            <td>{{ getTemperature(data.parameter[1].temp_min, data.parameter[1].temp_max) }}</td>
            <td>{{ getWeather(data.parameter[1].weather_day) }}</td>
            <td>{{ getWeather(data.parameter[1].weather_night) }}</td>
          </tr>
        </tbody>
      </table>
